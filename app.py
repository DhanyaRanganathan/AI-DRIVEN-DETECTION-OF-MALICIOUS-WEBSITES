...
# Check if URL is malicious and exists in history
existing = [h for h in history_data if h["site"] == url and h["risk"] == "HIGH"]
if existing:
    send_email(user_email=session['user'], url=url)

import smtplib
from email.mime.text import MIMEText

def send_email(user_email, url):
    sender_email = "your_email@gmail.com"
    sender_pass = "your_app_password"  # <-- use app password here

    subject = "⚠️ Malicious URL Alert"
    body = f"Warning! The URL {url} was previously flagged as malicious."
...
def predict_api():
    ...
    if not url:
        return {"result": "error", "message": "No URL provided"}
